...
var FTP = module.exports = function() {
  ...
  this.options = {
    ...
    passive: true,
    activeIp: undefined,
    activePortLo: undefined,
    activePortHi: undefined,
...
FTP.prototype.connect = function(options) {
  ...
  this.options.activePortLo = options.activePortLo || 41250;
  this.options.activePortHi = options.activePortHi || 41275;
  ...
  if(!this.options.passive){
    ...
    self.server.on('error', function(err) {
      if(err.code === 'EADDRINUSE') {
        setTimeout(function() {
          self.server.close();
          self._actvPort = self._actvPort + 1;
          if (self._actvPort > self.options.activePortHi) { self._actvPort = self.options.activePortLo; }
          self.server.listen(self._actvPort);
        }, 1000);
      }
      self.emit('error', new Error('Listening server error.'));
      self._reset();
    });
    self._actvPort = self.options.activePortLo;
    self.server.listen(self._actvPort, function() {
      self._actvPort = self.server.address().port;
...
FTP.prototype.get = function(path, zcomp, cb) {
  ...
  this._dataconn(function(err, sock) {
    ...
    if(self.options.passive){
      ...
      self._actvHandler = function(socket) {
        readFromSource(socket);
        socket.pause();
        cb(undefined, socket);
        socket.resume();
